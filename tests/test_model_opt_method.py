...
def test_ADVI(pcawg):
    ...
    model.fit(2)
    model = HierarchicalTandemLda(dataset=pcawg, n_damage_sigs=10, n_misrepair_sigs=5, type_col='tissue_type', opt_method = "ADVI")
    model.fit(2)
...
def test_FullRankADVI(pcawg):
    ...
    model.fit(2)
    model = HierarchicalTandemLda(dataset=pcawg, n_damage_sigs=4, n_misrepair_sigs=3, type_col='tissue_type', opt_method = "FullRankADVI")
    model.fit(2)